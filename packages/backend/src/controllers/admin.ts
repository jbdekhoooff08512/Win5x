...
router.get('/referrals/:userId/summary', requirePermission('MANAGE_USERS'), asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { userId } = req.params;
  const [l1, l2, l3, earningsAgg, recent] = await Promise.all([
    prisma.referral.count({ where: { parentId: userId, level: 1 } }),
    prisma.referral.count({ where: { parentId: userId, level: 2 } }),
    prisma.referral.count({ where: { parentId: userId, level: 3 } }),
    prisma.referralEarning.aggregate({ where: { userId }, _sum: { amount: true } }),
    prisma.referralEarning.findMany({ where: { userId }, orderBy: { createdAt: 'desc' }, take: 20 }),
  ]);
  res.json(createSuccessResponse({ levels: { l1, l2, l3 }, totalEarnings: earningsAgg._sum.amount || 0, recentEarnings: recent }));
}));

// Referral tree (L1, L2, L3 lists)
router.get('/referrals/:userId/tree', requirePermission('MANAGE_USERS'), asyncHandler(async (req: AuthenticatedRequest, res) => {
  const { userId } = req.params;
  const [l1Refs, l2Refs, l3Refs] = await Promise.all([
    prisma.referral.findMany({ where: { parentId: userId, level: 1 }, include: { user: { select: { id: true, username: true, createdAt: true } } } } as any),
    prisma.referral.findMany({ where: { parentId: userId, level: 2 }, include: { user: { select: { id: true, username: true, createdAt: true } } } } as any),
    prisma.referral.findMany({ where: { parentId: userId, level: 3 }, include: { user: { select: { id: true, username: true, createdAt: true } } } } as any),
  ]);
  res.json(createSuccessResponse({ l1: l1Refs, l2: l2Refs, l3: l3Refs }));
}));
