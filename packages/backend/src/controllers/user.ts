...
router.get('/referrals/summary', asyncHandler(async (req: AuthenticatedRequest, res) => {
...
// Referral progress for a specific minimum deposit (level-1 only)
router.get('/referrals/progress', asyncHandler(async (req: AuthenticatedRequest, res) => {
  const userId = req.user!.id;
  const minDepositParam = Number((req.query as any).minDeposit || 0);
  const minDeposit = isNaN(minDepositParam) ? 0 : Math.max(0, Math.floor(minDepositParam));

  // Fetch level-1 invitees
  const l1Refs = await prisma.referral.findMany({
    where: { parentId: userId, level: 1 },
    select: { userId: true },
  });
  ...
  if (inviteeIds.length === 0 || minDeposit <= 0) {
    return res.json(createSuccessResponse({ inviteesCount, qualifiedCount: 0 }));
  }

  // Count distinct invitees whose completed deposits meet the threshold
  const qualifiedUsers = await prisma.transaction.findMany({
    where: {
      userId: { in: inviteeIds },
      type: 'DEPOSIT',
      status: 'COMPLETED',
      amount: { gte: minDeposit },
    },
    distinct: ['userId'],
    select: { userId: true },
  });
  ...
  res.json(createSuccessResponse({ inviteesCount, qualifiedCount }));
}));
