...
export class PaymentService {
  ...
  async approveDepositRequest(
    ...
    // Deposit bonus to bonus wallet (only for Win5x coin deposits)
    try {
      const paymentMethod = await this.prisma.paymentMethod.findUnique({
        where: { id: deposit.paymentMethodId },
      });

      if (paymentMethod?.name === 'win5x-coin') {
        ...
        if (bonus > 0) {
          await WalletService.addBonus(deposit.userId, bonus, this.prisma);
          ...
          await this.prisma.transaction.create({
            data: {
              ...
              amount: bonus,
              status: 'COMPLETED',
              description: `Deposit bonus (${pct * 100}%) for Win5x Coin deposit`,
              approvedBy: adminId,
            },
          });
        }
    ...
    // Process referral rewards for multi-level referrals
    try {
      // Direct database calls for referral rewards
      // This will be implemented later if needed
      logger.info(`Referral rewards processing skipped for deposit: ${depositId}`, {
        userId: deposit.userId,
        amount: deposit.amount
      });
    } catch (e) {
