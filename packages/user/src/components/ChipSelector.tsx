...
const ChipSelector: React.FC<ChipSelectorProps> = ({
  ...
  useEffect(() => {
    if (!isCustomSelected && customAmount && !isCustomFocused) {
      setCustomAmount('');
      setCustomError('');
    }
  }, [isCustomSelected, customAmount, isCustomFocused]);
  ...
  return (
    <div className={`bg-gray-800 rounded-lg p-3 sm:p-4 border border-gray-700 shadow-lg ${className}`}>
        ...
        {predefinedChips.map((chip) => (
          <motion.button
            ...
            className={`p-2 sm:p-3 rounded-lg font-bold text-xs sm:text-sm transition-all ${
              selectedChip === chip && !isCustomSelected
      ...
      <div className="space-y-2">
        ...
        {/* Success Message */}
        {isCustomSelected && !customError && customAmount && (
          <motion.div
            initial={{ opacity: 0, y: -10 }}
            animate={{ opacity: 1, y: 0 }}
